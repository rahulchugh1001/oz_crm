            ...
            <div class="overflow-x-auto">
                <table class="min-w-[1300px] w-full table-fixed text-[13px]">
                    <thead class="border-b border-slate-200"
                        ...
                        <tr>
                            <th
                                class="w-[56px] px-4 py-2 text-left text-[10px] font-semibold text-white uppercase tracking-wider whitespace-nowrap">
                                #</th>
                            <th
                                class="w-[120px] px-4 py-2 text-center text-[10px] font-semibold text-white uppercase tracking-wider whitespace-nowrap">
                                Action</th>
                            <th
                                class="w-[150px] px-4 py-2 text-left text-[10px] font-semibold text-white uppercase tracking-wider whitespace-nowrap">
                                Date & Time</th>
                            <th
                                class="w-[100px] px-4 py-2 text-center text-[10px] font-semibold text-white uppercase tracking-wider whitespace-nowrap">
                                Type</th>
                            <th
                                class="w-[120px] px-4 py-2 text-left text-[10px] font-semibold text-white uppercase tracking-wider whitespace-nowrap">
                                Item Code</th>
                            <th
                                class="w-[180px] px-4 py-2 text-left text-[10px] font-semibold text-white uppercase tracking-wider whitespace-nowrap">
                                Item Name</th>
                            <th
                                class="w-[130px] px-4 py-2 text-left text-[10px] font-semibold text-white uppercase tracking-wider whitespace-nowrap">
                                Item Size</th>
                            <th
                                class="w-[120px] px-4 py-2 text-center text-[10px] font-semibold text-white uppercase tracking-wider whitespace-nowrap">
                                Quantity</th>
                            <th
                                ...
                                Transfer By</th>
                            <th
                                class="w-[200px] px-4 py-2 text-left text-[10px] font-semibold text-white uppercase tracking-wider whitespace-nowrap">
                                Remark</th>
                        </tr>
                    </thead>
                        ...
                        @forelse($pendingTransfers as $index => $transfer)
                            <tr class="hover:bg-slate-50 transition-colors">
                                @php
                                    ...
                                    $displayName = ($transfer->item_category === 'SF1-SF2' && !empty($transfer->item_name_sf2))
                                        ? $transfer->item_name_sf2
                                        : $transfer->item_name;
                                @endphp
                                    ...
                                    @if($canUpdateStatus)
                                        <div class="flex items-center justify-center gap-2">
                                            <button type="button"
                                                ...
                                                data-status="1" data-item-name="{{ $displayName }}"
                                                data-quantity="{{ (float) $transfer->quantity }}"
                                                data-type="{{ strtoupper($transfer->type) }}" data-date="{{ $transferDateTime }}"
                                                title="Accept">
                                ...
                                <td class="px-4 py-3 text-center">
                                    <span
                                        class="inline-flex items-center px-2 py-1 rounded-full text-[11px] font-medium bg-indigo-50 text-indigo-700">{{ strtoupper($transfer->type) }}</span>
                                </td>
                                ...
                                <td class="px-4 py-3 text-center">
                                    <span
                                        class="inline-flex items-center px-2 py-1 rounded-lg bg-blue-50 text-blue-700 text-xs font-semibold">
                                ...
                                <td colspan="10" class="px-4 py-10 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-slate-500 mt-1">There are no pending SF2 transfers to PPC at
                                                this time.</p>
                                        </div>
                                    </div>
                                </td>
            ...
            <form id="statusUpdateForm" method="POST" class="px-6 py-5 space-y-4">
                ...
                <div class="border border-slate-200 rounded-xl p-4 space-y-3">
                    ...
                    <div class="flex items-center gap-4">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="action_type" value="accept_all"
                                class="text-green-600 focus:ring-green-500" checked onchange="handleActionTypeChange()">
                            <span class="text-sm font-medium text-green-700">Accept All Quantity</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="action_type" value="partial_reject"
                                class="text-amber-600 focus:ring-amber-500" onchange="handleActionTypeChange()">
                            <span class="text-sm font-medium text-amber-700">Partially Reject</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="action_type" value="reject_all"
                                class="text-rose-600 focus:ring-rose-500" onchange="handleActionTypeChange()">
                ...
                <div id="reject_quantity_group" class="hidden">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label for="reject_quantity_field"
                                class="block text-sm font-semibold text-slate-700 mb-2">Reject Quantity</label>
                            <input type="number" id="reject_quantity_field" name="reject_quantity" min="0" step="1"
                                value="0"
                        ...
                        <div>
                            <label for="reject_reason_id_field"
                                class="block text-sm font-semibold text-slate-700 mb-2">Reject Reason</label>
                            <select id="reject_reason_id_field" name="reject_reason_id"
                                class="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div>
                    <label for="ppc_remark" class="block text-sm font-semibold text-slate-700 mb-2">PPC Remark
                        (Optional)</label>
                    <textarea id="ppc_remark" name="ppc_remark" rows="2" maxlength="500"
                        class="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        placeholder="Add remark here..."></textarea>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    ...
                    <button type="submit" id="status_confirm_button"
                        class="px-4 py-2 rounded-lg text-white font-medium transition-colors bg-green-600 hover:bg-green-700">Confirm
                        Process</button>
                </div>
            </form>
        </div>
