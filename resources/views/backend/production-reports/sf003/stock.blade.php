...
<div class="p-6">
    @php
        $canUpdateStatus = auth()->user()->role === 'SF003';
    @endphp
            ...
            <table class="min-w-[1700px] w-full table-fixed text-[13px]">
                <thead class="border-b border-slate-200" style="background: linear-gradient(to right, #141d30, #2d3a52);">
                    <tr>
                        ...
                        <th class="w-[110px] px-4 py-2 text-center text-[10px] font-semibold text-white uppercase tracking-wider whitespace-nowrap">SF3 Process</th>
                        <th class="w-[130px] px-4 py-2 text-center text-[10px] font-semibold text-white uppercase tracking-wider whitespace-nowrap">Received Quantity</th>
                ...
                <tbody class="divide-y divide-slate-200">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        @php
                            ...
                            $sf003Remark = trim((string) ($transfer->sf003_remark ?? ''));
                            $sf001ShortRemark = mb_strimwidth($sf001Remark, 0, 60, '...');
                            $sf003ShortRemark = mb_strimwidth($sf003Remark, 0, 60, '...');
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-2 flex-nowrap whitespace-nowrap">
                                <button
                                    ...
                                    data-sf3-process="{{ $transfer->sf3_process ?? '-' }}"
                                    ...
                                    data-sf003-remark="{{ $transfer->sf003_remark ?? '' }}"
                                    title="View Details"
                                ...
                                @if($canUpdateStatus)
                                @php
                                    $isUsedInSf3 = (int) ($transfer->is_used_in_sf3 ?? 0) === 1;
                                @endphp
                                <button
                                    type="button"
                                    class="inline-flex items-center justify-center p-2 text-[11px] font-medium rounded-lg transition-all {{ $isUsedInSf3 ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-green-50 text-green-700 hover:bg-green-100' }}"
                                    {{ $isUsedInSf3 ? '' : 'onclick=openStatusModal(this)' }}
                                    data-action="{{ route('admin.production-reports.sf003.stock.status', $transfer->id) }}"
                                    data-status="1"
                                    data-item-name="{{ $transfer->item_name }}"
                                    data-quantity="{{ (float) $transfer->quantity }}"
                                    data-sf3-process="{{ $transfer->sf3_process ?? '' }}"
                                    data-assigned-at="{{ $transfer->created_at ? \Carbon\Carbon::parse($transfer->created_at)->format('M d, Y h:i A') : '-' }}"
                                    data-current-remark="{{ $transfer->sf003_remark ?? '' }}"
                                    data-reject-quantity="{{ (float) ($transfer->reject_quantity ?? 0) }}"
                                    data-reject-reason-id="{{ $transfer->reject_reason_id ?? '' }}"
                                    title="{{ $isUsedInSf3 ? 'Stock used in SF3 production' : 'Accept' }}"
                                    aria-label="{{ $isUsedInSf3 ? 'Stock used in SF3 production' : 'Accept' }}"
                                    {{ $isUsedInSf3 ? 'disabled' : '' }}
                                >
                        ...
                        <td class="px-4 py-3 text-slate-700 align-top break-words">
                            @if($sf003Remark === '')
                                -
                            @elseif($sf003Remark === $sf003ShortRemark)
                                <span class="block max-w-[220px]">{{ $sf003Remark }}</span>
                            @else
                                <div class="max-w-[220px]">
                                    <span class="js-remark-short">{{ $sf003ShortRemark }}</span>
                                    <span class="js-remark-full hidden">{{ $sf003Remark }}</span>
                                    <button type="button" class="js-remark-toggle ml-1 text-[11px] font-medium text-blue-600 hover:text-blue-700">Read more</button>
                                </div>
                            @endif
                        </td>
    ...
    <div class="mx-auto mt-8 w-full max-w-4xl rounded-2xl bg-white shadow-xl border border-slate-200">
        ...
        <div class="px-6 py-5">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-3"><p class="text-[11px] uppercase tracking-wider text-slate-500">SF3 Process Type</p><p id="detailSf3Process" class="text-sm font-semibold text-slate-900 mt-1">-</p></div>
    ...
    <div class="mx-auto mt-10 w-full max-w-2xl rounded-2xl bg-white shadow-xl border border-slate-200">
        ...
        <form id="statusUpdateForm" method="POST" class="px-6 py-5 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-3">
                    <p class="text-[11px] uppercase tracking-wider text-slate-500">SF3 Process</p>
                    <p id="statusModalSf3Process" class="text-sm font-semibold text-slate-900 mt-1">-</p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-3">
            ...
            <div>
                <label for="status_sf003_remark" class="block text-sm font-semibold text-slate-700 mb-2">Assembly (SF3) Remark</label>
                <textarea id="status_sf003_remark" name="sf003_remark" rows="3" maxlength="500" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Add remark here..."></textarea>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const detailSf3Process = document.getElementById('detailSf3Process');
    ...
    const detailSf003Remark = document.getElementById('detailSf003Remark');
    ...
    const modalSf3Process = document.getElementById('statusModalSf3Process');
    const modalAssignedAt = document.getElementById('statusModalAssignedAt');
    const modalRemark = document.getElementById('status_sf003_remark');
    ...
    function getSf3ProcessLabel(processCode) {
    ...
    window.openDetailsModal = function(button) {
        ...
        detailSf3Process.textContent = getSf3ProcessLabel(button.getAttribute('data-sf3-process'));
        ...
        const sf003Remark = (button.getAttribute('data-sf003-remark') || '').trim();
        detailSf001Remark.textContent = sf001Remark || '-';
        detailSf003Remark.textContent = sf003Remark || '-';
    ...
    window.openStatusModal = function(button) {
        ...
        modalSf3Process.textContent = getSf3ProcessLabel(button.getAttribute('data-sf3-process'));
