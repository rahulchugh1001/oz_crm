                ...
                <table class="w-full border-collapse" id="productionTable">
                    <thead class="text-white" style="background: linear-gradient(to right, #141d30, #2d3a52);">
                        <tr>
                            ...
                            <th class="border border-slate-300 px-3 py-2 text-left text-[10px] font-semibold text-white min-w-64 whitespace-nowrap">Slide Size</th>
...
<script>
    ...
    function addMachineRow(machine, prefillData = null) {
        ...
        let prefillLabel = 'Select Size';
        slideSizes.forEach(size => {
            const selected = prefillData && size.id === prefillData.slide_size_id ? 'selected' : '';
            const label = `${size.name} (${size.size})`;
            if (selected) prefillLabel = label;
            sizeOptions += `<option value="${size.id}" ${selected}>${label}</option>`;
        });
        ...
        row.innerHTML = `
            ...
            <td class="border border-slate-300 px-3 py-2">
                <div class="slide-size-wrapper" data-machine-id="${machine.id}">
                    ...
                    <select name="slide_size_id[]" class="row-input slide-size-select" onchange="onSlideSizeChange(this)" required style="display:none;">
                        ${sizeOptions}
                    </select>
        ...
        // Wire up searchable dropdown and set pre-filled display text
        const addedRow = document.getElementById(`row-${machine.id}`);
        if (addedRow) {
            initSearchableDropdown(addedRow, prefillLabel);
        }
        ...
        return `
            <div class="ss-dropdown" id="ss-dropdown-${machineId}" data-open="false">
                <div class="ss-trigger" tabindex="0">
                    <span class="ss-display-text ss-placeholder">Select Size</span>
    ...
    function initSearchableDropdown(row, initialLabel) {
        ...
        // Set initial display label if a value is pre-selected
        if (initialLabel && initialLabel !== 'Select Size') {
            displayText.textContent = initialLabel;
