            ...
            <div class="p-6 border-b border-slate-200">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="text-lg font-bold text-slate-900">Roll Forming (SF1) Process - Production Report
                                Details</h2>
                            <p class="text-sm text-slate-500">Roll Forming (SF1) Report #{{ $productionReport->id }} -
                                {{ $productionReport->report_date }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        <a href="{{ route('admin.production-reports.edit', $productionReport) }}"
                        ...
                        <a href="{{ route('admin.production-reports.index') }}"
                            class="px-4 py-2 text-sm font-medium text-white rounded-lg hover:shadow-lg transition-all flex items-center gap-2"
                            style="background: linear-gradient(to right, #141d30, #2d3a52);">
                ...
                <div class="mb-6 pb-6 border-b border-slate-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div class="bg-slate-50 rounded-lg p-4">
                            ...
                            <p class="text-base font-semibold text-slate-900">{{ $productionReport->slideSize->name }}
                                ({{ $productionReport->slideSize->size }})</p>
                        </div>
                        <div class="bg-slate-50 rounded-lg p-4">
                        ...
                        <div class="bg-slate-50 rounded-lg p-4">
                            ...
                            <p class="text-base font-semibold text-slate-900">
                                {{ \Carbon\Carbon::parse($productionReport->report_date)->format('d M, Y') }}</p>
                        </div>
                        <div class="bg-slate-50 rounded-lg p-4">
                            <label class="block text-xs font-medium text-slate-500 mb-1">Shift</label>
                            <p class="text-base font-semibold text-slate-900">
                                <span
                                    class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $productionReport->shift == 'Morning' ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800' }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="bg-green-50 rounded-lg p-4 border border-green-200">
                            ...
                            <p class="text-3xl font-bold text-green-700">
                                {{ $productionReport->total_set_shift !== null ? number_format($productionReport->total_set_shift, 2) : '-' }}
                            </p>
                        </div>
                        <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
                            <label class="block text-xs font-medium text-blue-600 mb-1">Set/Hour (Target)</label>
                            <p class="text-3xl font-bold text-blue-700">
                                {{ $productionReport->set_per_hour !== null ? number_format($productionReport->set_per_hour, 2) : '-' }}
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        <div
                            ...
                            <div class="relative z-10">
                                <div class="flex items-center justify-between mb-2">
                                    <label
                                        class="block text-sm font-semibold {{ $isAchieved ? 'text-green-800' : 'text-red-800' }}">
                                        Target Status
                                    </label>
                                    @if($isAchieved)
                                ...
                                <div class="flex items-baseline gap-2 mb-1">
                                    <p
                                        class="text-3xl font-extrabold {{ $isAchieved ? 'text-green-700' : 'text-red-700' }}">
                                ...
                                <div class="mt-3 pt-3 border-t {{ $isAchieved ? 'border-green-200' : 'border-red-200' }}">
                                    ...
                                    <div class="flex justify-between items-center text-sm">
                                        <span
                                            class="{{ $isAchieved ? 'text-green-700' : 'text-red-700' }} font-medium flex items-center gap-1">
                                            <i data-lucide="zap" class="w-4 h-4"></i>
                                            Efficiency:
                                        </span>
                                        <span
                                            class="text-lg font-bold {{ $efficiency >= 100 ? 'text-green-700' : ($efficiency >= 80 ? 'text-yellow-600' : 'text-red-700') }}">
                            ...
                            <div class="absolute -right-5 -bottom-5 opacity-10">
                                @if($isAchieved)
                                    <i data-lucide="trophy"
                                        class="w-24 h-24 {{ $isAchieved ? 'text-green-600' : 'text-red-600' }}"></i>
                                @else
                            ...
                            <thead class="text-white" style="background: linear-gradient(to right, #141d30, #2d3a52);">
                                <tr>
                                    ...
                                    @foreach($timeLabels as $label)
                                        <th
                                            class="border border-slate-300 px-4 py-2 text-center text-[10px] font-semibold text-white min-w-24 whitespace-nowrap">
                                            {{ $label }}</th>
                                    @endforeach
                                </tr>
                            </thead>
                                ...
                                <tr class="bg-white">
                                    @foreach($hourFields as $field)
                                        <td class="border border-slate-300 px-4 py-3 text-center">
                                            <span
                                                class="text-lg font-semibold text-slate-900">{{ $productionReport->$field !== null ? number_format($productionReport->$field, 0) : '-' }}</span>
                                        </td>
                                    @endforeach
                                </tr>
                            </tbody>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div
                            ...
                            <p class="text-3xl font-bold text-emerald-700">
                                {{ number_format($productionReport->actual_set_shift, 2) }}</p>
                        </div>
                        <div
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div class="bg-slate-50 rounded-lg p-4">
                            ...
                            <p class="text-sm font-semibold text-slate-900">
                                {{ $productionReport->created_at->format('d M, Y') }}</p>
                        ...
                        <div class="bg-slate-50 rounded-lg p-4">
                            ...
                            <p class="text-sm font-semibold text-slate-900">
                                {{ $productionReport->updated_at->format('d M, Y') }}</p>
