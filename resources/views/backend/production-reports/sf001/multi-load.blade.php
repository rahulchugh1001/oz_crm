...
@section('content')
    <div id="fullPageLoader"
        class="fixed inset-0 z-[99999] hidden items-center justify-center bg-slate-900/40 backdrop-blur-sm transition-all">
    ...
    <div id="editCoilModal" class="hidden fixed inset-0 z-[100000] bg-slate-900/50 p-4 overflow-y-auto">
        <div class="mx-auto mt-10 mb-10 w-full max-w-5xl rounded-2xl bg-white shadow-xl border border-slate-200">
            <div class="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
                ...
                <button type="button" onclick="closeEditCoilModal()"
            ...
            <form id="editCoilForm" action="#" method="POST" class="px-6 py-5 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="edit_manufacture_id" class="block text-sm font-semibold text-slate-700 mb-2">Supplier
                            Name <span class="text-rose-500">*</span></label>
                        <select id="edit_manufacture_id" name="manufacture_id" required
                    ...
                    <div>
                        <label for="edit_process" class="block text-sm font-semibold text-slate-700 mb-2">Process <span
                                class="text-rose-500">*</span></label>
                        <select id="edit_process" name="process" required
                            class="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div>
                        ...
                        <input type="text" id="edit_coil_no" name="coil_no"
                            class="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>

                    <div>
                        <label for="edit_coil_size" class="block text-sm font-semibold text-slate-700 mb-2">Coil Size <span
                                class="text-rose-500">*</span></label>
                        <input type="text" id="edit_coil_size" name="coil_size" required
                            class="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>

                    <div>
                        <label for="edit_thickness" class="block text-sm font-semibold text-slate-700 mb-2">Thickness <span
                                class="text-rose-500">*</span></label>
                        <input type="number" id="edit_thickness" name="thickness" required min="0" step="0.001"
                            class="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>

                    <div>
                        <label for="edit_net_weight_kg" class="block text-sm font-semibold text-slate-700 mb-2">Net Weight
                            (KG) <span class="text-rose-500">*</span></label>
                        <input type="number" id="edit_net_weight_kg" name="net_weight_kg" required min="0" step="0.001"
                            class="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    <button type="button" onclick="closeEditCoilModal()"
                        class="px-4 py-2 rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50 transition-colors">Cancel</button>
                    <button type="submit"
                        class="px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 font-medium">Update
                        Coil</button>
                </div>
            </form>
        </div>
        ...
        <div class="mb-6 flex items-center justify-between">
            <div>
                <div class="flex items-center gap-3">
                    <h1 class="text-2xl font-black text-slate-800 tracking-tight">Manage Coil: <span
                            class="text-indigo-600">{{ $coil->coil_no ?: 'N/A' }}</span></h1>
                    <button type="button" onclick="openEditCoilModalFromMulti(this)" data-id="{{ $coil->id }}"
                        data-manufacture-id="{{ $coil->manufacture_id }}" data-coil-no="{{ $coil->coil_no }}"
                        data-coil-size="{{ $coil->coil_size }}" data-thickness="{{ $coil->thickness }}"
                        data-net-weight-kg="{{ $coil->net_weight_kg }}" data-process="{{ $coil->process }}"
                ...
                <p class="text-sm text-slate-500 mt-1">Manage weight distribution for Coil: <span
                        class="font-bold text-slate-700">{{ $coil->coil_no ?: 'Unnamed Coil' }}</span></p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.production-reports.sf001.coil-stock') }}"
                    class="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors shadow-sm">
            ...
            <div class="lg:col-span-1 space-y-6">
                <div class="rounded-2xl bg-white p-6 shadow-sm border border-slate-200">
                    ...
                    @php
                        ...
                        $totalConsumed = (float) $unloadedHistory->sum('consumed_weight');
                        $totalCoilWeight = (float) $coil->net_weight_kg + $totalAllocated + $totalConsumed;
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex justify-between items-center py-2 border-b border-slate-50">
                            ...
                            <span class="text-sm font-bold text-slate-900">{{ number_format($totalCoilWeight, 0) }}
                                KG</span>
                        </div>
                        <div class="flex justify-between items-center py-2 border-b border-slate-50">
                            <span class="text-sm text-slate-500">Available Stock</span>
                            <span class="text-sm font-bold text-emerald-600">{{ number_format($coil->net_weight_kg, 0) }}
                                KG</span>
                        </div>
                        <div class="flex justify-between items-center py-2">
                            <span class="text-sm text-slate-500">Distributed Weight</span>
                            <span class="text-sm font-bold text-indigo-600">{{ number_format($totalAllocated + $totalConsumed, 0) }}
                                KG</span>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 pt-6 border-t border-slate-100">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">Weight
                                Distribution</span>
                            <span class="text-xs font-bold text-indigo-600">{{ round($allocationPercentage) }}%
                                Allocated</span>
                        </div>
                ...
                <div class="rounded-2xl bg-white p-6 shadow-sm border border-slate-200 relative overflow-hidden">
                    ...
                    <form id="allocationForm"
                        action="{{ route('admin.production-reports.sf001.coil-stock.multi-load.store', $coil->id) }}"
                        method="POST"
                        class="space-y-4 {{ $coil->net_weight_kg <= 0 ? 'opacity-20 pointer-events-none select-none' : '' }}">
                        @csrf
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Target Machine</label>
                            <div class="machine-select-wrapper">
                                <!-- Hidden native select -->
                                <select name="machine_id" id="machine_id_hidden" required class="hidden-machine-select"
                                    style="display:none;">
                                ...
                                <div class="ss-dropdown" id="machine-search-dropdown" data-open="false">
                                    ...
                                    <div class="ss-panel">
                                        <div class="ss-search-wrap">
                                            ...
                                            <input type="text" class="ss-search-input" placeholder="Search machines..."
                                                autocomplete="off">
                                        </div>
                                        <ul class="ss-list"></ul>
                                    </div>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Coil No. <span
                                    class="text-rose-500">*</span></label>
                            <input type="text" name="coil_no" required placeholder="Enter Coil Number"
                                class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200/50 outline-none transition-all">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Allocation Weight (KG)</label>
                            <div class="relative">
                                <input type="number" name="allocated_weight" required min="1"
                                    max="{{ $coil->net_weight_kg }}"
                                    placeholder="Max: {{ number_format($coil->net_weight_kg, 0) }} KG"
                                    class="w-full rounded-xl border border-slate-300 pl-4 pr-12 py-2.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200/50 outline-none transition-all">
                        ...
                        <div>
                            ...
                            <input type="text" name="remark" placeholder="e.g. Initial load"
                                class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200/50 outline-none transition-all">
                        </div>
                        <button type="submit" id="submitBtn"
                            class="flex items-center justify-center gap-2 w-full rounded-xl bg-indigo-600 py-3 text-sm font-bold text-white shadow-lg shadow-indigo-200 hover:bg-indigo-700 hover:shadow-indigo-300 transition-all active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="rounded-2xl bg-white shadow-sm border border-slate-200 overflow-hidden">
                    <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                        ...
                        <span
                            class="inline-flex rounded-full bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-700 border border-indigo-100">{{ $allocatedMachines->count() }}
                            Machines</span>
                    </div>
                                    ...
                                    <tr class="hover:bg-slate-50/50 transition-colors">
                                        <td class="px-6 py-4">
                                            ...
                                            <p class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">
                                                {{ $allocation->machine->machine_code }}</p>
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            <span
                                                class="text-sm font-semibold text-slate-600">{{ $allocation->coil_no ?: 'N/A' }}</span>
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            <span
                                                class="text-sm font-bold text-slate-700">{{ number_format($allocation->allocated_weight, 0) }}
                                                KG</span>
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            <span
                                                class="text-sm font-bold text-indigo-600">{{ number_format($allocation->remaining_weight, 0) }}
                                                KG</span>
                                        </td>
                                        <td class="px-6 py-4">
                                            <span
                                                class="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-bold text-emerald-700 border border-emerald-100">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex items-center justify-center gap-2">
                                                <form
                                                    action="{{ route('admin.production-reports.sf001.coil-stock.multi-load.update', [$coil->id, $allocation->id]) }}"
                                                    method="POST" class="inline js-manage-form">
                                                    @csrf
                                                    <button type="button"
                                                        onclick="confirmManage(this, {{ $allocation->allocated_weight }})"
                                                        class="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition-colors text-xs font-bold"
                                                        title="Adjust Weight">
                                                ...
                                                <form
                                                    action="{{ route('admin.production-reports.sf001.coil-stock.multi-load.unload', [$coil->id, $allocation->id]) }}"
                                                    method="POST" class="inline js-unload-form">
                                                    @csrf
                                                    <button type="button" onclick="confirmUnload(this)"
                                                        class="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-100 transition-colors text-xs font-bold"
                                    ...
                                    <tr>
                                        <td colspan="6" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center gap-3">
                                                <div
                                                    class="h-12 w-12 rounded-full bg-slate-50 flex items-center justify-center text-slate-300">
                                                    <i data-lucide="inbox" class="w-6 h-6"></i>
                                                </div>
                                                <p class="text-slate-400 text-sm font-medium">No active machine allocations for
                                                    this coil.</p>
                                            </div>
                                        </td>
                                    </tr>
                                ...
                                @forelse($unloadedHistory as $history)
                                    <tr class="bg-slate-50/20 transition-colors">
                                        <td class="px-6 py-4">
                                            ...
                                            <p class="text-[9px] font-semibold text-slate-400 uppercase tracking-wider">
                                                {{ $history->machine->machine_code }}</p>
                                        </td>
                                        <td class="px-6 py-4">
                                            <span
                                                class="text-sm font-semibold text-slate-500">{{ $history->coil_no ?: 'N/A' }}</span>
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            <span
                                                class="text-sm font-bold text-slate-500 line-through decoration-slate-300">{{ number_format($history->allocated_weight, 0) }}
                                                KG</span>
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            <div class="inline-flex flex-col">
                                                <span
                                                    class="text-sm font-black text-rose-600">{{ number_format($history->consumed_weight, 0) }}
                                                    KG</span>
                                        ...
                                        <td class="px-6 py-4">
                                            <p class="text-xs font-bold text-slate-600">
                                                {{ $history->updated_at->format('d M, Y') }}</p>
                                            <p class="text-[10px] text-slate-400">{{ $history->updated_at->format('h:i A') }}
                                            </p>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-6 py-8 text-center text-slate-400 text-xs italic">No unloaded
                                            history yet.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                    ...
                    <div class="space-y-4">
                        @forelse($transitions as $track)
                            <div class="flex items-start gap-4 p-3 rounded-xl bg-slate-50/50 border border-slate-100">
                                <div
                                    class="mt-1 h-8 w-8 flex-shrink-0 flex items-center justify-center rounded-lg {{ $track->type === 'load' ? 'bg-indigo-50 text-indigo-600' : 'bg-rose-50 text-rose-600' }}">
                                    <i data-lucide="{{ $track->type === 'load' ? 'arrow-up-right' : 'arrow-down-left' }}"
                                        class="w-4 h-4"></i>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between gap-2">
                                        <p class="text-sm font-bold text-slate-800 capitalize">{{ $track->type }}ed on
                                            {{ $track->machine->name }}</p>
                                        <span
                                            class="text-[10px] font-medium text-slate-400">{{ $track->created_at->diffForHumans() }}</span>
                                    </div>
                                    <p class="text-xs text-slate-500 mt-0.5">
                                        Weight: <span
                                            class="font-bold {{ $track->type === 'load' ? 'text-indigo-600' : 'text-rose-600' }}">{{ number_format($track->load_weight, 0) }}
                                            KG</span>
                                        @if($track->loadNumber)
                                            <span class="mx-2 text-slate-300">|</span>
                                            <span class="font-semibold text-slate-600">Coil:
                                                {{ $track->loadNumber->coil_no }}</span>
                                        @endif
                                        @if($track->remark)
                                            <span class="mx-2 text-slate-300">|</span>
                        ...
                        @empty
                            <p class="text-xs text-slate-400 italic">No recent transitions for this multi-load configuration.
                            </p>
                        @endforelse
                    </div>
                </div>
    ...
    <style>
        ...
        .ss-option.ss-disabled { background: #f8fafc; color: #94a3b8; cursor: not-allowed; font-style: italic; }
        .ss-option.ss-disabled:hover { color: #94a3b8; }
        .ss-disabled-note { display: block; font-size: 10px; color: #f43f5e; font-weight: 600; font-style: normal; margin-top: 2px; }
            ...
            function populateList(filter = '') {
                ...
                options.forEach(opt => {
                    ...
                    if (isDisabled) {
                        ...
                        li.innerHTML = `
                            <div>${opt.text}</div>
                            <span class="ss-disabled-note ${noteClass}">${note}</span>
                        `;
                    } else {
