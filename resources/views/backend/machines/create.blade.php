        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-subtle">
            <div class="p-6 border-b border-slate-200">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-lg font-bold text-slate-900">Add New Machine</h2>
                        <p class="text-sm text-slate-500">Fill in the details below</p>
                    </div>
                </div>
            </div>

            <form id="machine-create-form" action="{{ route('admin.machines.store') }}" method="POST" class="p-6">
                @csrf
                ...
                <div>
                    <label for="status-toggle-create" class="block text-sm font-semibold text-slate-700 mb-2">
                ...
                <div>
                    <label for="ballcage-toggle-create" class="block text-sm font-semibold text-slate-700 mb-2">
                        Is Ballcage
                    </label>
                    <div class="px-4 py-3 rounded-lg">
                        <input type="hidden" name="is_ballcage" value="0">
                        <label class="status-toggle" for="ballcage-toggle-create">
                            <input
                                type="checkbox"
                                id="ballcage-toggle-create"
                                name="is_ballcage"
                                value="1"
                                data-ballcage-toggle
                                data-ballcage-text-id="ballcage-toggle-text-create"
                                {{ old('is_ballcage', '0') === '1' ? 'checked' : '' }}
                            >
                            <span class="status-toggle-track">
                                <span class="status-toggle-thumb"></span>
                            </span>
                            <span id="ballcage-toggle-text-create" class="status-toggle-text">No</span>
                        </label>
                    </div>
                    @error('is_ballcage')
                        <p class="mt-2 text-sm text-rose-600 flex items-center gap-1">
...
<script>
    ...
    (() => {
        const toggle = document.getElementById('ballcage-toggle-create');
        const label  = document.getElementById('ballcage-toggle-text-create');
    ...
    (() => {
        ...
        const defaultLabel = submitLabel ? submitLabel.textContent : 'Create Machine';
        ...
        form.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                if (response.ok) {
                    toastr.success((data.message || 'Machine created successfully.') + ' Redirecting to list page...');
                    setTimeout(() => {
...
<script>
    (() => {
        ...
        const initialSelected = @json(collect(old('weight_capacity', []))->toArray());
